...
def index():
	...
	if 'username' in session:
		conn = connect_db()
		user = conn.execute("SELECT id FROM student WHERE username = ?",(session['username'],)).fetchall()
		user = get_post_student(session['username'])
...
def show_profile():
	...
	if request.method == 'POST':
		...
		if request.form['action'] == 'continue':
			...
			return redirect(url_for('mess_preference'))
...
@app.route('/profile/mess_preference',methods=('GET','POST'))
def mess_preference():
...
def admin():
	...
	if request.method == 'POST':
		if request.form['action'] == 'mess':
			return redirect(url_for('admin_mess'))
...
@app.route('/admin/admin_mess',methods=('GET','POST'))
def admin_mess():
	...
	if request.method == 'POST':
		...
		if request.form['action'] == 'addmess':
			return redirect(url_for('admin_addmess'))
		if request.form['action'] == 'clearmess':
			...
			return redirect(url_for('admin_mess_details',messname=messname))
			
	return render_template('admin_mess.html',user=user, mess=mess, username=username)

@app.route('/admin/admin_mess/<messname>/admin_mess_details',methods=('GET','POST'))
def admin_mess_details(messname):
	...
	if request.method == 'POST':
		...
		if request.form['action'] == 'updatemess':
			return redirect(url_for('admin_updatemess', messname=messname))
	return render_template('admin_mess_details.html',mess=mess,student=student,username=username)

@app.route('/admin/admin_mess/<messname>/admin_mess_details/admin_updatemess',methods=('GET','POST'))
def admin_updatemess(messname):
	...
	if request.method == 'POST':
		...
		if request.form['action'] == 'save':
			...
			if category == "default" or name == "None" or owner == "None" or capacity == "None":
				...
				conn.commit()
				conn.close()
				return redirect(url_for('admin_mess_details', messname=name))
	return render_template('admin_updatemess.html', mess=mess,username=username)
	
@app.route('/admin/admin_mess/admin_addmess',methods=('GET','POST'))
def admin_addmess():
...
@app.route('/admin/admin_student',methods=('GET','POST'))
def admin_student():
...
@app.route('/admin/admin_student/add_student',methods=('GET','POST'))
def add_student():
...
@app.route('/admin/admin_student/<studname>/edit_student',methods=('GET','POST'))
def edit_student(studname):
	...
	if request.method == 'POST':
		...
		if request.form['action'] == 'clear':
			conn = connect_db()
			conn.execute("UPDATE student SET due = ? WHERE id = ?",(0, id))
			conn.commit()
			conn.close()
			return redirect(url_for('admin_student'))
